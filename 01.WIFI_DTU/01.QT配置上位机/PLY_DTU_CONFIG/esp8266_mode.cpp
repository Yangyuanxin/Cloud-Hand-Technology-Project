...
void MainWindow::DTU_FOR_ESP8266_CMD_REPLY_PROCESS()
{
    ...
    if(buf.contains("+DTUWFSAPSet:0"))
        ui->Setting_DTU_Tip->setText(tr("连接成功"));
    else if(buf.contains("+DTUWFSAPSet:1"))
        ui->Setting_DTU_Tip->setText(tr("连接失败"));
    ...
    if(buf.contains("+DTUWFSAP?:"))
    {
        ...
        memcpy(buf_ssid,ptr+1,len-1);
        buf_ssid[len-1] = '\0';
        qDebug() << "SAPSSID:" << buf_ssid ;
        qDebug()<<"buf:" << buf ;
        char *ptr2 = strstr(buf.toLatin1(),",");
        qDebug()<<"ptr2:" << ptr2 ;

        ptr1 = ptr2 ;
        ptr = strstr((char *)ptr2, ",");
        ptr1 = strstr((char *)ptr2, "\r\n");
        len = ptr1-ptr ;
#endif
        char *ptr = strstr(buf.toLatin1(),":");
        qDebug() << ptr ;
        //memcpy(buf_pwd,ptr+1,len-1);
        //buf_pwd[len-1] = '\0';
        ui->Current_SoftAp->setText(tr("%1").arg(ptr+strlen("+DTUWFSAP?:")));
    }
}
